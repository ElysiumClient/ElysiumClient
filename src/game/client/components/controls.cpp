...
int CControls::SnapInput(int *pData)
{
	...
	else
	{
		...
		m_aFastInputHookAction[g_Config.m_ClDummy] = false;
		m_aFastInputFireAction[g_Config.m_ClDummy] = false;
...
// ec_fast_input: compares the current live input state (for both dummy slots) against
// the last captured snapshot. Returns true (and updates the snapshots) if anything
// meaningful changed on either slot, so the caller knows to repredict immediately
// instead of waiting for the next tick. Ported from AetherClient's CheckNewInput().
bool CControls::CheckNewInput()
{
	...
	for(int Dummy = 0; Dummy < NUM_DUMMIES; Dummy++)
	{
		...
		{
			m_aFastInputHookAction[Dummy] = true;
			SetMousePos = true;
		}
		if(m_aFastInput[Dummy].m_Fire != TestInput.m_Fire && TestInput.m_Fire % 2 == 1)
		{
			m_aFastInputFireAction[Dummy] = true;
			SetMousePos = true;
		}
		if(!m_aFastInputHookAction[Dummy] && !m_aFastInputFireAction[Dummy])
			SetMousePos = true;
